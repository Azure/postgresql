...
class SingleParentDir {
    ...
    getFileList() {
        ...
        ;
        core.debug(`ak - filePathRegex: ${this.filePathRegex}`);
        core.debug(`current dir fileS: ${fs.readdirSync("./")}`);
        core.debug(`current dir fileS: ${fs.readdirSync("./" + "/" + "this.filePathRegex")}`);
        let listOfMatchedFiles = glob.sync(this.filePathRegex, {});
        core.debug(`ak - listOfMatchedFiles: ${this.listOfMatchedFiles}`);
